...
class stock:
    ...
    def step(self, action, show_log=False, my_trick=False):
        ...
        ret = (self.maket_value - self.last_value)/self.last_value
        self.stock_volatility = np.sqrt(self.stock_volatility**2 * 0.8 + ret**2*0.2) 
        ...
        reward = reward - 10*self.stock_volatility**2 
    ...
    def draw(self, save_name1, save_name2):
        ...
        n = len(profit_rate_account)
        daily_ret = (np.array(profit_rate_account[3:n])+1)/(np.array(profit_rate_account[2:(n-1)])+1) - 1
        baseline_ret = (np.array(profit_rate_stock[3:n])+1)/(np.array(profit_rate_stock[2:(n-1)])+1) - 1
        print('account annulized volatility is',np.std(np.nan_to_num(daily_ret))*np.sqrt(252))
        print('stock annulized volatility is',np.std(np.nan_to_num(baseline_ret))*np.sqrt(252))
        print('Account profit is',profit_rate_account[-1])
        fig = plt.figure(figsize = (15,5))
